...
export async function saveProRiderSeason(formData: FormData) {
  ...
  // Same fallback as the CSV import — current_salary defaults to
  // starting_salary at initial setup, since there's no history yet.
  const newCurrentSalary =
    parseSalary(formData.get("current_salary")) ?? startingSalary;
